## LA CTF 2026 - single-trust Write-up

![challenge info](images/Title.png)

### Step 1: The "Fix" That Broke Everything

The challenge description states that the author "patched" a backdoor from a previous challenge. We are provided with a `diff` showing the critical change in the Node.js code:
...
**The Vulnerability:**
The author removed `cipher.final()`.
In AES-GCM (Galois/Counter Mode), `cipher.final()` is the step that verifies the **Auth Tag** (integrity check). Without this call, the server decrypts whatever ciphertext we send it without verifying if it has been tampered with.

This effectively downgrades the authenticated encryption to a standard Stream Cipher (CTR mode), where:

 **Ciphertext = Plaintext ⊕ Keystream**


### Step 2: Exploitation Strategy (Math Time)

Since the server ignores integrity, we can perform a **Known Plaintext Attack** (Bit-Flipping) to rewrite the internal JSON.

1. **Recover the Keystream:**
We know the valid session cookie starts with a predictable JSON structure: `{"tmpfile":"/tmp/pastestore/`.
We extract the ciphertext from our cookie and XOR it with this known prefix:

 **Keystream = OriginalCiphertext ⊕ KnownPlaintext**


2. **Forge the Payload:**
We want the server to read `/flag.txt`.
Our target JSON is: `{"tmpfile":"/flag.txt"}`.
We encrypt this by XORing it with the recovered keystream:

 **NewCiphertext = TargetPlaintext ⊕ Keystream**

3. **The Tag Brute-force:**
Even though `final()` is missing, `crypto.createDecipheriv` still expects an Auth Tag. The script brute-forces a 1-byte tag to satisfy the library's buffer requirements.
...
The script recovers the keystream, forges the ciphertext pointing to `/flag.txt`, and bypasses the tag check.
